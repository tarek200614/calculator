...
class CalculatorUI:
    ...
    def _build_button_grid(self) -> None:
        ...
        for row_index, row in enumerate(BUTTON_ROWS):
            # A row with a single entry (just the "=" button) spans the
            # full width; every other row has one button per column.
            col_span = num_cols if len(row) == 1 else 1
    ...
    def _on_history_select(self, _event: tk.Event) -> None:
        ...
        if not selection:
            return
        # The listbox is populated most-recent-first (see _refresh_history),
        # while calculator.history.all() returns entries oldest-first, so we
        # must reverse before indexing or the wrong entry gets loaded.
        entries = list(reversed(self.calculator.history.all()))
        index = selection[0]
